  ...
  return (
    ...
    >
      <a
        ...
        className={`group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white/90 shadow-md transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-lg ${carouselTile ? 'max-h-[26rem]' : ''}`}
      >
        ...
        <div className="flex flex-1 flex-col p-4">
          <span className="text-[10px] font-bold uppercase tracking-wide text-sky-700/90">
            {article.source}
          </span>
          <h3
            className={`mt-1 line-clamp-2 font-semibold leading-snug text-text-dark group-hover:text-primary-end ${carouselTile ? 'text-sm' : 'text-base'}`}
          >
            {article.title}
          </h3>
          {article.description ? (
            <p
              className={`mt-2 line-clamp-3 leading-relaxed text-text-dark/65 ${carouselTile ? 'text-xs' : 'text-sm'}`}
            >
              {article.description}
            </p>
          ) : null}
          <time className="mt-auto pt-3 text-[11px] text-text-dark/45" dateTime={article.publishedAt || undefined}>
